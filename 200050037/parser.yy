...
function_definition:  
    ...
    { 
	  //cout << "type_specifier fun_declarator" << endl;
...
type_specifier:
    VOID {
      //cout << "void type spec" << endl;
      $$ = new type_specifier_class(VOID_TYPE);
    } 
  | INT {
      //cout << "int type spec" << endl;
      $$ = new type_specifier_class(INT_TYPE);
    } 
  | FLOAT {
      //cout << "float type spec" << endl;
    ...
    IDENTIFIER '(' ')' ';'
{
		...
		SymbTabEntry *temp = gst.search($1);
		if (temp -> type.structname != "") {istruct = true; issize = gst.Entries[temp -> type.structname].size;}
		$$ = new funcall_astnode(new identifier_astnode($1, temp->offset, temp->size, temp -> type.is_array, temp -> type.deref, istruct, issize));
		istruct = false; issize = 0;
		if (predefined.find($1) == predefined.end())
...
postfix_expression: 
  ...
  | postfix_expression '[' expression ']'
  {
		...
		for (int i=0; i < (int)($1->data_type.array.size()); i++){
			//cout << $1->data_type.array[i];
		}
		aoffs = 1;
		for (int i=1; i < (int)($1->data_type.array.size()); i++){
			aoffs = aoffs * $1->data_type.array[i];
		}
		$$ = new arrayref_astnode($1, $3, aoffs);
  ...
  | postfix_expression '.' IDENTIFIER {
		...
		if (identifier_entry -> type.structname != "") {istruct = true; issize = gst.Entries[identifier_entry -> type.structname].size;}
		$$ = new member_astnode($1, new identifier_astnode($3, identifier_entry -> offset, identifier_entry -> size, identifier_entry -> type.is_array, identifier_entry -> type.deref, istruct, issize)); // Add in lex
		istruct = false; issize = 0;
  ...
  | postfix_expression PTR_OP IDENTIFIER{
		...
		if (identifier_entry -> type.structname != "") {istruct = true; issize = gst.Entries[identifier_entry -> type.structname].size;}
		$$ = new arrow_astnode($1, new identifier_astnode($3, identifier_entry -> offset, identifier_entry -> size, identifier_entry -> type.is_array, identifier_entry -> type.deref, istruct, issize)); // Add in lex
		istruct = false; issize = 0;
		$$->data_type = identifier_entry->type;
		$$->lvalue = true;
  }
...
primary_expression:
    IDENTIFIER {
		...
		if (temp -> type.structname != "") {istruct = true; issize = gst.Entries[temp -> type.structname].size;}
		$$ = new identifier_astnode($1, temp->offset, temp->size, temp -> type.is_array, temp -> type.deref, istruct, issize);
		istruct = false; issize = 0;
		$$->data_type = temp->type;
		$$->lvalue = true;
}
  ...
  | STRING_LITERAL {
		//cout << "line 1454" << endl;
		$$ = new stringconst_astnode($1,numpf); // did not set the data type for primary_expression here
		$$->lvalue = false;
